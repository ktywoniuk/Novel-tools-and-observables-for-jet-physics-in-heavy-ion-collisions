...
For a full-fledged jet, the diagram is built up by mapping every branching to a point on the Lund plane. In \autoref{fig:PS0} (right), we illustrate how to fill this plane for multiple, {\sl primary} emissions off the main branch, in this case three. Resolving splitting of each branch will, in turn, generate new, orthogonal Lund planes, and so on. Simplifying the graphical representation, these planes are ultimately collapsed onto the original one. Due to the strong ordering of emissions in the double-logarithmic regime, especially in angles, the emissions are roughly evenly distributed in the splitting variables.
...
Hence, each splitting in \autoref{fig:PS0} (right) happens approximately at $t_{\text{f},i}$ ($i=\{1,2,3\}$).

Turning now to medium effects, it is most natural to consider which of the splittings happen inside a medium of length $L$.  The line corresponding to $\tform = L$ is found by substituting $t = L$ in \eqref{eq:FormationTimeLund}, and is also represented in \autoref{fig:PS1} picking a particular value for $L$. Hence, the area above the line marked $\tform = L$ correspond to emissions that occur inside the medium. Emissions with $\tform > L$ occupy the region below the line.
...
\eeq
Note that the slope is smaller than in \eqref{eq:FormationTimeLund}.
...
The kinematical Lund plane for {\sl one} splitting inside the medium is therefore divided into three main regimes, in addition to the possibility to fragmenting outside of the medium, $\tform > L$.
Emissions that fall within the red area in \autoref{fig:PS1}, i.e. with $\tform< \tdecoh < L$, correspond to pure vacuum splittings inside the medium. On the other hand, splittings with $\tdecoh > L$, falling within the blue triangle in \autoref{fig:PS1}, are never resolved by medium interactions.
...
A direct comparison with fully-fledged dynamical models, employed in \autoref{sec:phasespace-mc}, is still not directly applicable. We hope, nevertheless, that this discussion serves as a useful sketch for separating out regimes dominated, within the scope of the current discussion, by widely different physical mechanisms.
...
The figures produced in this report were built using the code at \cite{LundFromMC}.
